...
export default function Users() {
  ...
  useEffect(()=>{load();api.getTenants().then(result=>setTenants(result.data)).catch(()=>{});}, []);
  ...
  return <div>
    ...
    <form className="card" onSubmit={create} style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10 }}>
        ...
        <select className="form-select" value={form.tenantId||''} onChange={e=>setForm({...form,tenantId:e.target.value})}><option value="">Equipe global do NOC</option>{tenants.map(item=><option key={item.id} value={item.id}>Cliente: {item.name}</option>)}</select>
      </div>
    ...
    <div className="card">
      ...
      <div style={{ overflowX: 'auto' }}><table style={{ width:'100%', borderCollapse:'collapse' }}><thead><tr><th>Nome</th><th>Usuário</th><th>Escopo</th><th>Perfil</th><th>2FA</th><th>Estado</th><th>Último acesso</th><th>Ações</th></tr></thead><tbody>
        {users.map(u => <tr key={u.id} style={{ borderTop:'1px solid var(--border-primary)' }}><td style={{padding:10}}>{u.name}</td><td>{u.username}</td><td><select className="form-select" value={u.tenantId||''} onChange={e=>update(u.id,{tenantId:e.target.value||null})}><option value="">NOC global</option>{tenants.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></td><td><select className="form-select" style={{minWidth:150}} value={u.role} onChange={e => update(u.id,{role:e.target.value})}><option value="admin">Administrador</option><option value="operator">Operador NOC</option><option value="viewer">Visualização</option></select></td><td><span className={`badge ${u.twoFactorEnabled?'badge-success':'badge-warning'}`}>{u.twoFactorEnabled?'Ativo':'Inativo'}</span></td><td><button className={`btn ${u.isActive?'btn-success':'btn-secondary'}`} onClick={() => update(u.id,{isActive:!u.isActive})}>{u.isActive?'Ativo':'Inativo'}</button></td><td>{u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleString('pt-BR') : 'Nunca'}</td><td style={{display:'flex',gap:6,padding:10}}><button className="btn btn-secondary" onClick={() => { const password=window.prompt('Nova senha forte temporária (mínimo 10 caracteres):'); if(password) update(u.id,{password}); }}><Save size={14}/> Senha</button><button className="btn btn-secondary" onClick={() => remove(u)}><Trash2 size={14}/></button></td></tr>)}
      </tbody></table></div>

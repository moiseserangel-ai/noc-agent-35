  ...
  return <div>
    ...
    {loading ? <div className="loading-screen" style={{minHeight:240}}><div className="spinner"/></div> : data && <>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:16,marginBottom:16}}>{data.providers.map(item=><div className="card" key={item.provider}>
        ...
        {item.cooldownUntil&&<div className="alert alert-warning" style={{marginTop:14,fontSize:'.78rem'}}><AlertTriangle size={15}/><div>Bloqueado até {date(item.cooldownUntil)}.<br/>{item.reason}</div></div>}
      </div>)}</div>
      <div className="card" style={{marginBottom:16,fontSize:'.78rem',color:'var(--text-muted)'}}>{data.pricingNote}</div>
      <div className="table-container"><table><thead><tr><th>Data</th><th>Provedor/modelo</th><th>Agente</th><th>Resultado</th><th>Tokens</th><th>Custo estimado</th><th>Duração</th></tr></thead><tbody>{data.recent.map(row=><tr key={row.id}><td>{date(row.createdAt)}</td><td><strong>{providerName[row.provider] || row.provider}</strong><br/><span style={{color:'var(--text-muted)',fontSize:'.75rem'}}>{row.model}</span></td><td>{row.agentName || '—'}</td><td>{row.status==='success'?<span className="status-badge status-completed">Sucesso</span>:<><span className="status-badge status-failed">{errorLabel[row.errorKind] || row.status}</span>{row.errorMessage&&<div title={row.errorMessage} style={{maxWidth:280,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',fontSize:'.72rem',marginTop:4}}>{row.errorMessage}</div>}</>}</td><td>{number(row.totalTokens)}</td><td>{row.estimatedCost===null?'—':money(row.estimatedCost)}</td><td>{row.durationMs===null?'—':`${row.durationMs} ms`}</td></tr>)}{!data.recent.length&&<tr><td colSpan="7">Ainda não há chamadas registradas.</td></tr>}</tbody></table></div>
    </>}
  </div>;
}

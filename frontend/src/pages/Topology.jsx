...
const rate=value=>value===null||value===undefined?'—':value>=1e9?`${(value/1e9).toFixed(2)} Gbps`:value>=1e6?`${(value/1e6).toFixed(2)} Mbps`:value>=1e3?`${(value/1e3).toFixed(1)} Kbps`:`${Math.round(value)} bps`;
const vendorIcon = manufacturer => ({mikrotik:'MT', huawei:'HW', cisco:'CS', juniper:'JN', ubiquiti:'UB'}[(manufacturer||'').toLowerCase()] || 'NW');
const Sparkline=({points,field,color='#2563eb',max})=>{const values=points.map(point=>point[field]),available=values.filter(value=>value!==null&&value!==undefined);if(!available.length)return <div className="link-chart-empty">Sem dados no período</div>;const ceiling=max||Math.max(...available,1),coords=values.map((value,index)=>value===null||value===undefined?null:`${points.length===1?150:index/(points.length-1)*300},${74-Math.min(value/ceiling,1)*66}`).filter(Boolean).join(' ');return <svg className="link-sparkline" viewBox="0 0 300 82" preserveAspectRatio="none"><line x1="0" y1="74" x2="300" y2="74"/><polyline points={coords} style={{stroke:color}}/></svg>};
...
export default function Topology({isAdmin=false}){
 ...
 const [showDiscovery,setShowDiscovery]=useState(false);
  ...
  const [telemetryPeriod,setTelemetryPeriod]=useState(24);
  ...
  useEffect(()=>{if(!selectedLink){setTelemetryHistory(null);return}let active=true;api.getTopologyLinkTelemetry(selectedLink,telemetryPeriod).then(result=>{if(active)setTelemetryHistory(result.data)}).catch(error=>{if(active)toast(error.message,'error')});return()=>{active=false}},[selectedLink,telemetryPeriod,toast]);
  ...
  const collectLinkTelemetry=async()=>{
    setBusy(true);
    try{const result=await api.collectTopologyTelemetry();toast(result.message,'success');await load(true);}catch(error){toast(error.message,'error');}finally{setBusy(false);}
  };
  const saveLinkPolicy=async event=>{
    event.preventDefault();if(!selectedLinkData)return;setBusy(true);
    const form=new FormData(event.currentTarget),payload={monitoringEnabled:form.get('monitoringEnabled')==='on',warningUtilization:Number(form.get('warningUtilization')),criticalUtilization:Number(form.get('criticalUtilization')),warningLatencyMs:Number(form.get('warningLatencyMs')),warningPacketLoss:Number(form.get('warningPacketLoss')),confirmationSamples:Number(form.get('confirmationSamples')),recoverySamples:Number(form.get('recoverySamples'))};
    try{const result=await api.updateTopologyLinkPolicy(selectedLinkData.id,payload);toast(result.message,'success');await load(true);}catch(error){toast(error.message,'error')}finally{setBusy(false)}
  };
  ...
  return <div className="topology-page">
    ...
    <div className="page-header page-header-actions"><div><h2>Mapa de rede</h2><p>Topologia operacional, disponibilidade e incidentes em tempo real</p></div><div>
      ...
      {isAdmin&&<button className="btn btn-secondary" disabled={busy} onClick={collectLinkTelemetry}><Activity size={15}/> Coletar links</button>}
    ...
    <div className={'topology-workspace '+(fullscreen?'topology-workspace-fullscreen':'')}>
          ...
          <div className="topology-canvas" style={{width:canvas.width,height:canvas.height,transform:`scale(${zoom})`}} onPointerDown={panDown} onPointerMove={panMove} onPointerUp={panUp} onPointerCancel={panUp}>
            ...
            <svg className="topology-links" width={canvas.width} height={canvas.height} onPointerDown={panDown} onPointerMove={panMove} onPointerUp={panUp} onPointerCancel={panUp}>
              {visibleLinks.map(link=>{const a=positions[link.sourceDeviceId],b=positions[link.targetDeviceId];if(!a||!b)return null;const affected=impact&&(impactedIds.has(link.sourceDeviceId)||impactedIds.has(link.targetDeviceId));return <g key={link.id} className={`topology-link ${link.linkType} health-${link.status} ${impact&&!affected?'impact-dim':''} ${selectedLink===link.id?'selected':''} ${linkDiff(link.id)}`} onClick={()=>{setSelectedLink(link.id);setSelected(null)}}>
                <line x1={a.x+82} y1={a.y+42} x2={b.x+82} y2={b.y+42} style={link.telemetry?.utilization!==null&&link.telemetry?.utilization!==undefined?{strokeWidth:Math.min(9,2.5+link.telemetry.utilization/20)}:undefined}/>
                ...
                {(link.telemetry?.utilization!==null&&link.telemetry?.utilization!==undefined||link.label||link.bandwidthMbps)&&<text className="topology-link-label" x={(a.x+b.x)/2+82} y={(a.y+b.y)/2+27} textAnchor="middle">{link.telemetry?.utilization!==null&&link.telemetry?.utilization!==undefined?pct(link.telemetry.utilization):link.label||`${link.bandwidthMbps} Mbps`}</text>}
              </g>})}
            </svg>
      ...
      {selectedLinkData&&!selectedNode&&<aside className="card topology-detail topology-link-detail"><button className="topology-detail-close" onClick={()=>setSelectedLink(null)}>×</button><div className={`topology-detail-state ${selectedLinkData.status}`}>{STATUS[selectedLinkData.status]?.label||selectedLinkData.status}</div><h3>{selectedLinkData.sourceNode?.name} ↔ {selectedLinkData.targetNode?.name}</h3><p>{selectedLinkData.statusSource==='zabbix'?'Telemetria atual do Zabbix':'Estado inferido pelas duas pontas'}</p><dl><div><dt>Origem</dt><dd>{selectedLinkData.sourceInterface||'—'}</dd></div><div><dt>Destino</dt><dd>{selectedLinkData.targetInterface||'—'}</dd></div><div><dt>Tipo</dt><dd>{selectedLinkData.linkType}</dd></div><div><dt>Capacidade</dt><dd>{selectedLinkData.telemetry?.capacityBps?rate(selectedLinkData.telemetry.capacityBps):selectedLinkData.bandwidthMbps?`${selectedLinkData.bandwidthMbps} Mbps`:'Não informada'}</dd></div><div><dt>Entrada</dt><dd>{rate(selectedLinkData.telemetry?.inputBps)}</dd></div><div><dt>Saída</dt><dd>{rate(selectedLinkData.telemetry?.outputBps)}</dd></div><div><dt>Utilização</dt><dd>{pct(selectedLinkData.telemetry?.utilization)}</dd></div><div><dt>Latência</dt><dd>{selectedLinkData.telemetry?.latencyMs!==null&&selectedLinkData.telemetry?.latencyMs!==undefined?`${selectedLinkData.telemetry.latencyMs} ms`:'—'}</dd></div><div><dt>Perda</dt><dd>{pct(selectedLinkData.telemetry?.packetLoss)}</dd></div><div><dt>Última coleta</dt><dd>{date(selectedLinkData.telemetry?.collectedAt)}</dd></div><div><dt>Descoberta</dt><dd>{selectedLinkData.source}</dd></div></dl><section className="link-history"><header><strong>Histórico</strong><select value={telemetryPeriod} onChange={event=>setTelemetryPeriod(Number(event.target.value))}><option value="1">1 hora</option><option value="24">24 horas</option><option value="168">7 dias</option><option value="720">30 dias</option></select></header>{telemetryHistory&&<><div className="link-history-summary"><span><b>{pct(telemetryHistory.summary.averageUtilization)}</b> média</span><span><b>{pct(telemetryHistory.summary.maximumUtilization)}</b> pico</span><span><b>{telemetryHistory.summary.saturationEvents}</b> saturações</span></div><label>Tráfego de entrada</label><Sparkline points={telemetryHistory.points} field="inputBps" color="#2563eb"/><label>Tráfego de saída</label><Sparkline points={telemetryHistory.points} field="outputBps" color="#7c3aed"/><label>Utilização</label><Sparkline points={telemetryHistory.points} field="utilization" color="#d97706" max={100}/><label>Latência</label><Sparkline points={telemetryHistory.points} field="latencyMs" color="#16a34a"/></>}</section>{isAdmin&&<details className="link-policy"><summary>Política de alertas</summary><form key={selectedLinkData.id} onSubmit={saveLinkPolicy}><label className="link-policy-enabled"><input name="monitoringEnabled" type="checkbox" defaultChecked={selectedLinkData.monitoringEnabled}/> Monitoramento automático</label><div><label>Utilização alerta (%)<input name="warningUtilization" type="number" min="1" max="100" defaultValue={selectedLinkData.warningUtilization}/></label><label>Utilização crítica (%)<input name="criticalUtilization" type="number" min="1" max="100" defaultValue={selectedLinkData.criticalUtilization}/></label><label>Latência (ms)<input name="warningLatencyMs" type="number" min="1" defaultValue={selectedLinkData.warningLatencyMs}/></label><label>Perda (%)<input name="warningPacketLoss" type="number" min="0.1" max="100" step="0.1" defaultValue={selectedLinkData.warningPacketLoss}/></label><label>Confirmar após<input name="confirmationSamples" type="number" min="2" max="12" defaultValue={selectedLinkData.confirmationSamples}/></label><label>Recuperar após<input name="recoverySamples" type="number" min="2" max="12" defaultValue={selectedLinkData.recoverySamples}/></label></div><button className="btn btn-primary btn-sm" disabled={busy}>Salvar política</button></form></details>}{isAdmin&&<button className="btn btn-danger" onClick={()=>removeLink(selectedLinkData.id)}>Remover conexão</button>}</aside>}
    </div>}

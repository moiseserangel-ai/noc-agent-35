...
export default function Compliance() {
  ...
  return <div>
    ...
    {profilesOpen&&<div className="modal-overlay" onClick={()=>{setProfilesOpen(false);setEditingProfile(null);}}><div className="modal compliance-profiles-modal" onClick={event=>event.stopPropagation()}><div className="modal-header"><div><h3>Perfis e regras</h3><p>Personalize controles por fabricante sem alterar os equipamentos</p></div><button className="btn btn-ghost" onClick={()=>{setProfilesOpen(false);setEditingProfile(null);}}><X size={18}/></button></div>
      {!editingProfile?<><div className="compliance-profile-create"><input className="form-input" placeholder="Nome do novo perfil" value={newProfile.name} onChange={e=>setNewProfile({...newProfile,name:e.target.value})}/><select className="form-select" value={newProfile.deviceType} onChange={e=>setNewProfile({...newProfile,deviceType:e.target.value})}><option value="mikrotik">MikroTik RouterOS</option><option value="huawei_vrp">Huawei VRP</option><option value="cisco_ios">Cisco IOS / IOS-XE</option><option value="juniper_junos">Juniper Junos</option><option value="fortigate_fortios">Fortinet FortiGate / FortiOS</option><option value="ubiquiti_edgeos">Ubiquiti EdgeOS</option><option value="datacom_dmos">Datacom DMOS</option><option value="nokia_sros">Nokia SR OS</option><option value="linux">Linux</option></select><button className="btn btn-primary" onClick={createProfile}><Plus size={15}/> Criar</button></div><div className="compliance-profile-list">{profiles.map(profile=><button key={profile.id} onClick={()=>setEditingProfile(structuredClone(profile))}><div><strong>{profile.name}</strong><span>{typeLabel[profile.deviceType]} · {profile.rules.filter(rule=>rule.enabled).length} regras ativas · meta {profile.minimumScore}%</span></div>{profile.isSystem&&<span className="status-badge status-completed">Padrão</span>}</button>)}</div></>:<>

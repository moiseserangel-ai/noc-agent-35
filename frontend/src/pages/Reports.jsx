...
export default function Reports() {
  ...
  const exportCsv = () => {
    if (!report) return;
    const header = ['Task','Abertura','Resolução','Equipamento','Fonte','Tipo','Prioridade','Status','MTTA segundos','MTTR segundos','SLA violado'];
    const rows = report.incidents.map(x => [x.taskNumber,x.createdAt,x.resolvedAt||'',x.device,x.source,labels[x.workType]||x.workType,x.priority,x.status,x.mttaSeconds??'',x.mttrSeconds??'',x.slaBreached?'Sim':'Não']);
  ...
  return <div>
    ...
    {loading ? <div className="loading-screen" style={{minHeight:240}}><div className="spinner"/></div> : report && <>
      <div className="stats-grid" style={{marginBottom:16}}>
        {[['Total de atividades',report.summary.total],['Incidentes',report.workTypes.incidents],['Consultas',report.workTypes.consultations],['Configurações',report.workTypes.configurations],['Em aberto',report.summary.open],['Resolvidos',report.summary.resolved],['MTTA',duration(report.summary.mttaSeconds)],['MTTR',duration(report.summary.mttrSeconds)],['SLA resolução',report.summary.slaCompliance===null?'—':`${report.summary.slaCompliance}%`],['SLA violado',report.summary.slaBreached]].map(([name,value])=><div className="stat-card" key={name}><div className="stat-icon"><BarChart3 size={20}/></div><div><div className="stat-value">{value}</div><div className="stat-label">{name}</div></div></div>)}
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:16,marginBottom:16}}><Bars title="Por tipo de atividade" data={report.byWorkType}/><Bars title="Por prioridade" data={report.byPriority}/><Bars title="Por status" data={report.byStatus}/><Bars title="Por origem" data={report.bySource}/></div>
      <div className="card" style={{marginBottom:16}}><h3 style={{marginBottom:14}}>Tendência diária</h3><div style={{display:'flex',alignItems:'end',gap:4,height:190,overflowX:'auto',paddingTop:15}}>{report.daily.map(day=><div key={day.date} title={`${day.date}: ${day.incidents} incidentes, ${day.consultations} consultas, ${day.configurations} configurações`} style={{minWidth:24,flex:1,height:'100%',display:'flex',alignItems:'end',gap:2,borderBottom:'1px solid var(--border-primary)'}}><div style={{width:'33%',height:`${day.incidents/maxDaily*100}%`,minHeight:day.incidents?3:0,background:'var(--danger)',borderRadius:'3px 3px 0 0'}}/><div style={{width:'33%',height:`${day.consultations/maxDaily*100}%`,minHeight:day.consultations?3:0,background:'var(--primary)',borderRadius:'3px 3px 0 0'}}/><div style={{width:'33%',height:`${day.configurations/maxDaily*100}%`,minHeight:day.configurations?3:0,background:'var(--warning)',borderRadius:'3px 3px 0 0'}}/></div>)}</div><div style={{display:'flex',gap:16,marginTop:8,fontSize:'.75rem',flexWrap:'wrap'}}><span>🔴 Incidentes</span><span>🔵 Consultas</span><span>🟡 Configurações</span></div></div>

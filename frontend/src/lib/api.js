...
export const api = {
  ...
  getSuppliers: (params={}) => request(`/suppliers?${new URLSearchParams(params)}`),
  ...
  getCommercialContracts: (params={}) => request(`/commercial-contracts?${new URLSearchParams(params)}`),
  createCommercialContract: data => request('/commercial-contracts',{method:'POST',body:JSON.stringify(data)}),
  updateCommercialContract: (id,data) => request(`/commercial-contracts/${id}`,{method:'PUT',body:JSON.stringify(data)}),
  createContractRenewalTask: id => request(`/commercial-contracts/${id}/renewal-task`,{method:'POST'}),
  getSoftwareLicenses: (params={}) => request(`/software-licenses?${new URLSearchParams(params)}`),
  getSoftwareLicenseSummary: (params={}) => request(`/software-licenses/summary?${new URLSearchParams(params)}`),
  createSoftwareLicense: data => request('/software-licenses',{method:'POST',body:JSON.stringify(data)}),
  updateSoftwareLicense: (id,data) => request(`/software-licenses/${id}`,{method:'PUT',body:JSON.stringify(data)}),
  createLicenseRenewalTask: id => request(`/software-licenses/${id}/renewal-task`,{method:'POST'}),
  getCommercialExpiryAutomation: () => request('/software-licenses/automation'),
  setCommercialExpiryAutomation: autoTasks => request('/software-licenses/automation',{method:'PUT',body:JSON.stringify({autoTasks})}),
  runCommercialExpiryMonitor: () => request('/software-licenses/monitor',{method:'POST'}),
  getCommercialCosts: (params={}) => request(`/commercial-costs?${new URLSearchParams(params)}`),
  getCommercialDashboard: (params={}) => request(`/commercial-dashboard?${new URLSearchParams(params)}`),
  downloadCommercialReport: async(format,params={})=>{const res=await fetch(`${BASE}/commercial-dashboard/export.${format}?${new URLSearchParams(params)}`,{headers:{Authorization:`Bearer ${getToken()}`}});if(!res.ok){let data={};try{data=await res.json()}catch{}throw new Error(data.error||'Falha ao exportar relatório comercial')}return{blob:await res.blob(),filename:res.headers.get('content-disposition')?.match(/filename="([^"]+)"/)?.[1]||`relatorio-comercial.${format}`}},
  ...
  createCmdbInterface: (id,data) => request(`/cmdb/${id}/interfaces`,{method:'POST',body:JSON.stringify(data)}),
  deleteCmdbInterface: (id,interfaceId) => request(`/cmdb/${id}/interfaces/${interfaceId}`,{method:'DELETE'}),
  getCmdbVlans: id => request(`/cmdb/${id}/vlans`),
  createCmdbVlan: (id,data) => request(`/cmdb/${id}/vlans`,{method:'POST',body:JSON.stringify(data)}),
  deleteCmdbVlan: (id,vlanId) => request(`/cmdb/${id}/vlans/${vlanId}`,{method:'DELETE'}),
  ...
  deleteCmdbRelationship: id => request(`/cmdb/relationships/${id}`,{method:'DELETE'}),
  ...
  getIpam: (params={}) => request(`/cmdb/ipam?${new URLSearchParams(params)}`),
  ...
  importIpam: rows => request('/cmdb/ipam/import',{method:'POST',body:JSON.stringify({rows})}),
  deleteIpamSubnet: id => request(`/cmdb/ipam/subnets/${id}`,{method:'DELETE'}),
  createIpamAddress: data => request('/cmdb/ipam/addresses',{method:'POST',body:JSON.stringify(data)}),
  deleteIpamAddress: id => request(`/cmdb/ipam/addresses/${id}`,{method:'DELETE'}),
  getIpamSuggestions: () => request('/cmdb/ipam/suggestions'),
  refreshIpamSuggestions: () => request('/cmdb/ipam/suggestions/refresh',{method:'POST'}),
  approveIpamSuggestion: id => request(`/cmdb/ipam/suggestions/${id}/approve`,{method:'POST'}),
  ignoreIpamSuggestion: id => request(`/cmdb/ipam/suggestions/${id}/ignore`,{method:'POST'}),
  getBusinessServices: () => request('/cmdb/business-services'),
  createBusinessService: data => request('/cmdb/business-services',{method:'POST',body:JSON.stringify(data)}),
  updateBusinessService: (id,data) => request(`/cmdb/business-services/${id}`,{method:'PUT',body:JSON.stringify(data)}),
  deleteBusinessService: id => request(`/cmdb/business-services/${id}`,{method:'DELETE'}),
  linkBusinessServiceAsset: (id,data) => request(`/cmdb/business-services/${id}/assets`,{method:'POST',body:JSON.stringify(data)}),
  unlinkBusinessServiceAsset: (id,assetId) => request(`/cmdb/business-services/${id}/assets/${assetId}`,{method:'DELETE'}),
  getBusinessServiceImpact: id => request(`/cmdb/business-services/${id}/impact`),
  ...
  createTopologySnapshot: baseline => request('/topology/history',{method:'POST',body:JSON.stringify({baseline})}),
  ...
  prepareRunbookRollback: id => request(`/runbooks/executions/${id}/rollback/prepare`,{method:'POST'}),
  rollbackRunbook: (id,preparationId) => request(`/runbooks/executions/${id}/rollback`,{method:'POST',body:JSON.stringify({confirmed:true,preparationId})}),
  ...
  testNetboxIntegration: () => request('/integrations/netbox/test', { method:'POST' }),
  ...
  uploadChatAttachment: (id,data) => request(`/chat/sessions/${id}/attachments`,{method:'POST',body:JSON.stringify(data)}),
  deleteChatAttachment: id => request(`/chat/attachments/${id}`,{method:'DELETE'}),
  exportChatSession: async(id,format)=>{const res=await fetch(`${BASE}/chat/sessions/${id}/export.${format}`,{headers:{Authorization:`Bearer ${getToken()}`}});if(!res.ok){let data={};try{data=await res.json()}catch{}throw new Error(data.error||'Falha ao exportar conversa')}return{blob:await res.blob(),filename:res.headers.get('content-disposition')?.match(/filename="([^"]+)"/)?.[1]||`chat-ai.${format}`}},

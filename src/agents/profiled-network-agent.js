...
class ProfiledNetworkAgent extends BaseAgent{
  ...
  async diagnose(deviceId,deviceName,request,taskNumber){try{return await this.run(`Diagnostique ${deviceName} (ID ${deviceId}) na Task #TASK-${taskNumber}. Solicitação: ${request}. Use deviceId "${deviceId}".`);}catch(error){logger.error(error.message);return{text:`❌ ${error.message}`,toolsUsed:[]};}}

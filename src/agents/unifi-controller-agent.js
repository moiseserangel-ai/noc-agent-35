...
export default class UniFiControllerAgent extends BaseAgent{
  ...
  async diagnose(deviceId,deviceName,request,taskNumber){try{return await this.run(`Consulta UniFi para ${deviceName} (ID: ${deviceId}), Task #TASK-${taskNumber}. Solicitação: ${request}. Use deviceId "${deviceId}".`);}catch(error){logger.error(`[unifi_controller] ${error.message}`);return{text:`❌ Erro ao consultar ${deviceName}: ${error.message}`,toolsUsed:[]};}}

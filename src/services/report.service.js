...
const distribution = (tasks, key) => Object.entries(tasks.reduce((acc, task) => { const value = task[key] || 'unknown'; acc[value] = (acc[value] || 0) + 1; return acc; }, {})).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
...
export async function buildIncidentReport(query = {}) {
  ...
  const mttaValues = tasks.filter(task => task.acknowledgedAt).map(task => Math.max(0, Math.round((new Date(task.acknowledgedAt) - new Date(task.incidentOpenedAt || task.createdAt)) / 1000)));
  const mttrValues = resolved.filter(task => task.resolvedAt).map(task => Math.max(0, Math.round((new Date(task.resolvedAt) - new Date(task.incidentOpenedAt || task.createdAt)) / 1000)));
  const slaEligible = resolved.filter(task => task.slaResolveDueAt && task.resolvedAt);
  const slaCompliant = slaEligible.filter(task => new Date(task.resolvedAt) <= new Date(task.slaResolveDueAt));
  const ackEligible = tasks.filter(task => task.slaAckDueAt && task.acknowledgedAt);
  const ackCompliant = ackEligible.filter(task => new Date(task.acknowledgedAt) <= new Date(task.slaAckDueAt));

  const dailyMap = new Map();
  for (let cursor = new Date(period.from); cursor <= period.to; cursor = new Date(cursor.getTime() + 86_400_000)) dailyMap.set(dayKey(cursor), { date: dayKey(cursor), opened: 0, resolved: 0 });

...
export async function notifyCommercialExpiry(alert,io=null){
  const cfg=await getNotificationConfig(),icon=alert.severity==='critical'?'🚨':'⚠️',text=[`${icon} ${alert.kind==='license'?'Licença':'Contrato'} — vencimento`,alert.title,alert.message,cfg.baseUrl?`Revisar: ${cfg.baseUrl}/${alert.kind==='license'?'software-licenses':'commercial-contracts'}`:null].filter(Boolean).join('\n'),event=`${alert.kind}_expiry_${alert.stage}`,resource=`commercial:${alert.kind}:${alert.id}:${alert.expiryDate}`,meta={title:alert.title,message:text,priority:alert.severity,resourceType:`commercial_${alert.kind}`,resourceId:alert.id},results=[];
  if(await recordStandalone(resource,event,'panel',null,'sent',null,meta)){io?.emit('commercial:notification',{resourceId:alert.id,event,message:text});results.push({channel:'panel',status:'sent'});}
  ...
  for(const channel of [...new Set(channels)])for(const recipient of channel==='telegram'?cfg.telegramChats:[null]){const key=`${resource}:${event}:${channel}:${recipient||'default'}`;if(await prisma.notificationLog.findUnique({where:{dedupKey:key}}))continue;try{if(channel==='telegram')await sendTelegramMessage(recipient,text,cfg.telegramToken);else if(channel==='whatsapp'){const sent=await sendToAdmin(text);if(!sent)throw new Error('WhatsApp Admin não configurado');}else continue;await recordStandalone(resource,event,channel,recipient,'sent',null,meta);results.push({channel,status:'sent'});}catch(error){await recordStandalone(resource,event,channel,recipient,'failed',error.message,meta);results.push({channel,status:'failed'});}}
  return results;
}

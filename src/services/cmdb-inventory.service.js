...
export async function collectCmdbInventory(assetId,{username='system',type='manual'}={}){
  ...
  try{
    ...
    await prisma.cmdbAsset.update({where:{id:asset.id},data:{manufacturer:data.manufacturer||asset.manufacturer,model:data.model||asset.model,serialNumber:data.serialNumber||asset.serialNumber,hostname:data.hostname||asset.hostname,lastInventoryAt:now,lastInventoryStatus:'success',lastInventoryError:null,inventoryData:JSON.stringify(data),updatedBy:username}});
